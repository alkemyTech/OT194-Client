	...
	return (
		...
		<div className="flex flex-col mx-auto" style={{ maxWidth: '1200px' }} >
			...
			<div className="my-2 w-full overflow-hidden flex items-center " style={{ height: 'fit-content', maxHeight: '500px' }}>
				<Slider arr={newsDetails?.data?.Slides || []}/>
			</div>
			<div className="flex flex-col px-5 md:px-8">
				<h2 className="font-sans">{ newsDetails?.data?.name || 'Se ha producido un error inesperado'}</h2>
				<p className="m-0 p-0">{ newsDetails?.data?.content || 'La novedad solicitad no fue encontrada.'}</p>
