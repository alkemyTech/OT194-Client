		...
		<table className="mx-auto shadow-lg bg-white  border border-slate-400 ">
			<thead className='bg-redOng'>
				<tr>
					...
					<th className='bg-blue-100 border text-left px-8 py-4 '>Created At</th>
			...
			<tbody>
				{
					data.map((item, i) =>	(
						<tr key={i}>
							...
							<td className='border px-8 py-4 '><img className='rounded-full h-12 w-12' src={item.image}/></td>
							<td className='border px-8 py-4 '>{item.cratedAt}</td>
							<td className=''><Button className='border-0 px-6 py-2 rounded bg-blueOng text-white cursor-pointer' cb={handleNewsUpdate.bind(null, item.id)} text='Update'/></td>
							<td className=''><Button className='border-0 px-6 py-2 rounded bg-redOng text-white cursor-pointer' cb={handleNewsDelete.bind(null, item.id)} text='Delete'/></td>
						</tr>))
				}
